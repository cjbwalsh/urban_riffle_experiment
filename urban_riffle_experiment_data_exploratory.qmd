...
I saved the data from an habitat experiment in 9 streams in eastern Melbourne into an excel worksheet (see `urban_riffle_exp_data_compilation.qmd`), which stores the 6 tables read in the code chunk below. You'll see there is an extra step to turn the sites table into a spatial (simple features, sf) table.
...
biota <- data.frame(readxl::read_excel(dat_file, sheet = "biota"), 
                    stringsAsFactors = FALSE)
taxonomy <- data.frame(readxl::read_excel(dat_file, sheet = "taxonomy"), 
                    stringsAsFactors = FALSE)
taxon_all <- data.frame(readxl::read_excel(dat_file, sheet = "taxon_all"), 
                    stringsAsFactors = FALSE)
metadata <- data.frame(readxl::read_excel(dat_file, sheet = "metadata"), 
                    stringsAsFactors = FALSE)
