setwd("~/git/urban_riffle_experiment")
...
samples$ba <- 0
samples$ba[substr(samples$old_samplecode,1,1) == "3"] <- 2
samples$ba[substr(samples$old_samplecode,1,1) == "4"] <- 3
samples$ba[substr(samples$old_samplecode,1,1) == "5"] <- 8
samples$ba[substr(samples$old_samplecode,1,1) == "6"] <- 11
# 0 = before, trips 3, 4, 5, 6  = 2,3,8,11, respectively
samples$ba <- as.vector(scale(samples$ba))
...
stanfit_i$save_output_files(
  ...
  basename = "fit_nbinom_ba23811_cii_2000iter_ll", timestamp = FALSE, random = FALSE)
stanfit_i$sampler_diagnostics()
saveRDS(stanfit_i, file = "~/uomShare/wergStaff/ChrisW/git-data/urban_riffle_experiment/model_fits/fit_nbinom_ba23811_cii_2000iter_ll.rds")
...
min(summ$ess_bulk,na.rm=TRUE) # 126 from 1000 iterations
min(summ$ess_tail,na.rm=TRUE) # 173
