...
#### NEED TO CHECK subsample perc for THESE 4 on lab sheets
...
re_db <- RSQLite::dbConnect(RSQLite::SQLite(),
...
samples <- RSQLite::dbReadTable(re_db,"samples")
biota <- RSQLite::dbReadTable(re_db,"biota")
taxonomy <- RSQLite::dbReadTable(re_db,"taxon_tab")
taxon_all <- RSQLite::dbReadTable(re_db,"taxon_all")
metadata <- RSQLite::dbReadTable(re_db,"metadata")
...
# Adjustments to consider, still undecided
...
biota <- biota[biota$taxoncode != "KC349999",]
...
biota_ct <- ct(biota$smpcode, biota$shortcode, biota$naive)
biota_ct <- data.frame(sample = row.names(biota_ct),
                       site = samples$sitecode[match(row.names(biota_ct),
                                                     samples$smpcode)],
                       biota_ct)
...
WriteXLS::WriteXLS(list(biota = biota_ct, taxa = taxa), "~/uomShare/wergStaff/ChrisW/git-data/urban_riffle_experiment/biota_ct.xlsx")
```
