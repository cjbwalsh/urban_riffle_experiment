...
sites$ti <- imp_subcs$c_ti[match(sites$site, imp_subcs$site)]
...
#### Eummemmering survey in paper form:
#### https://photos.app.goo.gl/n9UtYEgCCWE2WJfo7
# From this, slope appears to be 1.7% (higher than other sites except HVD. Check these...)
# channel width at five transects = c(4.5,2.9,2.5,2.1,2) # mean 2.8 sd 1.0
# floodplain height = ? similar to HVD, I think...
...
metadata1 <- data.frame(table_name = "sites", field = names(sites),
      ...
      "Total imperviousness Dec 2004, from mwstr database",
...
mean_depth_vel$vel_m_s[is.na(mean_depth_vel$vel_m_s)] <- NA
...
for(i in 1:nrow(subs_56)){
  ...
  samples$max_phi[samples$old_samplecode == subs_56$smpcode[i]] <- 
    substrate_codes$phi[substrate_codes$substrate_code == subs_56$max_code[i]]
  samples$med_phi[samples$old_samplecode == subs_56$smpcode[i]] <- 
    substrate_codes$phi[substrate_codes$substrate_code == subs_56$max_code[i]]
}
...
WriteXLS::WriteXLS(list(sites = sites, samples = samples, biota = biota, taxon_all = taxon_all, taxa = taxa, higher_taxa = higher_taxa, metadata = metadata),
                   "~/uomShare/wergStaff/ChrisW/git-data/urban_riffle_experiment/urban_riffle_experiment_data_for_model.xlsx")
# On review, it became apparent that the organic matter dry weight data from samples was unreliable 
# (Distinct analyst differences between trips 1-4 and 5-6). I thus chose to remove those data from consideration
datlist <- rio::import_list("~/uomShare/wergStaff/ChrisW/git-data/urban_riffle_experiment/urban_riffle_experiment_data_for_model.xlsx")
datlist$samples <- datlist$samples[!grepl("_g",names(datlist$samples))]
metadata_samples <- datlist$metadata[datlist$metadata$table_name == "samples",]
metadata <- datlist$metadata[datlist$metadata$table_name != "samples",]
metadata_samples <- metadata_samples[metadata_samples$field %in% names(datlist$samples),]
metadata <- rbind(metadata, metadata_samples)
datlist$metadata <- metadata
WriteXLS::WriteXLS(datlist,
    "~/uomShare/wergStaff/ChrisW/git-data/urban_riffle_experiment/urban_riffle_experiment_data_for_model.xlsx")
...
terra::writeRaster(ia, "data/ia_5m_raster.tif")
streams <- sf::st_crop(streams, sf::st_bbox(sf::st_buffer(cats,1e4)))
pipes <- sf::st_crop(pipes, sf::st_bbox(sf::st_buffer(cats,1e4)))
victoria <- sf::st_read("data/Victoria_GDA94_GCS.gpkg", quiet = TRUE)
australia <- sf::st_read("data/Australia_GDA94_GCS.gpkg", quiet = TRUE)
sf::st_write(streams, "data/urban_riffle_experiment_map.gpkg", layer = "streams")
sf::st_write(pipes, "data/urban_riffle_experiment_map.gpkg", layer = "pipes", append = TRUE)
sf::st_write(cats, "data/urban_riffle_experiment_map.gpkg", layer = "cats", append = TRUE)
sf::st_write(coast, "data/urban_riffle_experiment_map.gpkg", layer = "coast", append = TRUE)
sf::st_write(victoria, "data/urban_riffle_experiment_map.gpkg", layer = "victoria", append = TRUE)
sf::st_write(australia, "data/urban_riffle_experiment_map.gpkg", layer = "australia", append = TRUE)
```
