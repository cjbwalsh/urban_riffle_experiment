...
ni <- 8500; nt <- 4; nb <- 2000; nc <- 4
stanfit_i <- mod$sample(data = sdata,
                        seed = rand_seed, chains = nc,
                        parallel_chains = nc, iter_warmup = nb,
                        adapt_delta = 0.9,  #default of 0.8 resulted in BFMI slightly low
                        iter_sampling = ni - nb, refresh = 85)
...
stanfit_i$save_output_files(
  ...
  basename = "fit_riffle_baci_mt_ad", timestamp = FALSE, random = FALSE)
stanfit_i$sampler_diagnostics()
saveRDS(stanfit_i, file = "~/uomShare/wergStaff/ChrisW/git-data/urban_riffle_experiment/model_fits/fit_riffle_baci_mt_ad.rds")
...
# (confirmed with stanfit_i$cmdstan_diagnose() that EBFMI is below nominal threshold of 0.3)
...
snames(predy_cf) <- names(biota_ct)
...
bgs[names(biota_ct) %in% unique(c(ba2ci_neg_taxa,ba0cii_neg_taxa))] <- "green"
...
for(t in 1:sdata$n_taxa){
    obs_per_sample[t,] <- aggregate(biota_ct[,t], 
                                     ...
                                     FUN = function(y) {as.numeric(sum(y > 0) > 0)})$x
  }
