data {
  ...
  array[n_obs] int samp_no;   // Sample number (equivalent vectorised [t_no, site_no])
...
parameters {
  ...
  vector[n_sample] a_st_raw;  // coefficient of random sample effect (1 sample per site_no per t_no)
...
model {
  // Priors
   ...
   sigma_s ~ student_t(2, 0, 0.1); //uniform(.001,10); //exponential(1); //normal(0.5,2); 
   sigma_t ~ student_t(2, 0, 0.1); //uniform(.001,10); //exponential(1); //normal(0.5,2);  
