...
stanfit_i <- mod$sample(data = mt_data,
                        ...
                        adapt_delta = 0.95,
                        iter_sampling = 5050, refresh = 100) 
# 5000 iterations took ~4 h
# #  save csv files rather than the model object to use less RAM
...
Finally, we assessed the accuracy of the multi-taxon model's estimates of assemblage level metrics, by calculating the *R^2^* and slope of the linear regression between predicted and observed number of taxa in each sample, both total number of taxa, and the number of Ephemeropotera, Plecoptera and Trichoptera taxa (three groups of sensitive species commonly used as a stream health metric). Predicted number of taxa was estimated as the number of taxa with probability of occurrence $\ge$0.5. Because observed taxon richness is likely to be affected by subsample size, we interpreted the relationship between predicted and observed numbers of taxa accounting for mean subsample proportion in the subsample units making up each sample. We also calculated *R^2^* of the predicted number of taxa as a function of observed number of taxa + mean subsample proportion.
...
The model also predicted occurrence of all taxa with high certainty (all AUC values of the 111 taxa absent from at least one sample $\ge$0.90, @fig-nbinom_pa_stats). The probabilities of occurrence of all taxa summed to accurate estimates of taxon richness in the 53 samples (@fig-richness_op). The raw correlation between predicted and observed numbers of all taxa was reduced by the effect of subsampling (@fig-richness_op A). The predicted number of taxa was higher than the observed for samples with lower numbers of taxa because more of them had small subsample fractions compared to more taxa-rich samples. The regression of predicted \~ observed + subsample fraction had a higher *R^2^* (0.75). The model predicted EPT richness with high accuracy (*R^2^* 0.96), with a smaller effect of subsample fraction (@fig-richness_op B).
...
# for case of all ps being equal
p_per_n1 <- function(x,n) {1 - (1 - x)^n} # x, a vector of probabilities
...
# An alternative way to estimate richness: 
...
pa_mat <- list()
for(i in 1:mt_data$n_taxa){
  # convert signal_mat to presence-absence assuming P > 0.5 is a presence
  pa_mat[[i]] <- t(apply(p_per_samp_mat[[i]], 1, function(x) round(x)))
  }

pa1_mat <- p_per_samp_mat
for(i in 1:mt_data$n_taxa){
  # convert signal_mat to presence-absence using rbinom(1,1,0)
    for(k in 1:ncol(pa1_mat[[i]])){
  pa1_mat[[i]][,k] <- rbinom(nrow(pa1_mat[[i]]),1,p_per_samp_mat[[i]][,k])
    }
  }

tot_rich_mat <- Reduce("+",p_per_samp_mat)
tot_rich_mat1 <- Reduce("+",pa_mat)
tot_rich_mat2 <- Reduce("+",pa1_mat)

par(mfrow = c(2,2));par(mar = c(4,4,1,1))
plot(apply(tot_rich_mat,2,FUN = mean), ylim = c(20,50),
      apply(p_mean_per_sample,2, FUN = sum),
     ylab = "Sum of mean probabilities per sample",
     xlab = "Mean of summed probabilities per sample")
abline(0,1)
title(main = "A.", adj = 0)

plot(apply(tot_rich_mat1,2,FUN = mean), ylim = c(20,50),
      apply(p_mean_per_sample,2, FUN = sum),
     xlab = "Mean number of taxa per sample (P >= 0.5)",
     ylab = "Sum of mean probabilities per sample")
abline(0,1)
title(main = "B.", adj = 0)

plot(apply(tot_rich_mat1,2,FUN = mean), ylim = c(20,50),
      apply(tot_rich_mat,2,FUN = mean),
     xlab = "Mean number of taxa per sample (P >= 0.5)",
     ylab = "Mean of summed probabilities per sample")
abline(0,1)
title(main = "C.", adj = 0)

plot(apply(tot_rich_mat,2,FUN = mean), ylim = c(20,50),
      apply(tot_rich_mat2,2,FUN = mean),
     xlab = "Sum of mean probabilities per sample",
     ylab = "Mean number of taxa per sample (rbinom)")
abline(0,1)
title(main = "D.", adj = 0)
...
y <- apply(p_mean_per_sample[substr(names(p_mat),1,2) %in% c("QE","QP","QT"),],2, 
           FUN = sum)
