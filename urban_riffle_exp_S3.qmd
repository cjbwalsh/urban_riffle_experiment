...
# Dan's method
# Before scenarios c(1,4,55,58)
# A1 scenarios c(2,5,56,59) but with A1 set to zero
predx_cf_dg <- predx_cf[c(1,4,55,58,2,5,56,59),]
predx_cf_dg$ba1[5:8] <- 0
cf_draws_dg <- 
      drawsi[,"gamma[1]"] %*% t(predx_cf_dg$`(Intercept)`) +
      drawsi[,"gamma[2]"] %*% t(predx_cf_dg$ba1) +
      drawsi[,"gamma[3]"] %*% t(predx_cf_dg$ba2) +
      drawsi[,"gamma[4]"] %*% t(predx_cf_dg$ci) +
      drawsi[,"gamma[5]"] %*% t(predx_cf_dg$i) + 
      drawsi[,"gamma[6]"] %*% t(predx_cf_dg$`ba1:ci`) +
      drawsi[,"gamma[7]"] %*% t(predx_cf_dg$`ba2:ci`) +
      drawsi[,"gamma[8]"] %*% t(predx_cf_dg$`ba0:ci:i`) + 
      drawsi[,"gamma[9]"] %*% t(predx_cf_dg$`ba1:ci:i`) +
      drawsi[,"gamma[10]"] %*% t(predx_cf_dg$`ba2:ci:i`) 
x <- data.frame(predx_cf_dg, 
             mean = apply(cf_draws_dg,2, mean),
             t(as.data.frame(apply(cf_draws_dg, 2,
