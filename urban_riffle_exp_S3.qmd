...
# adjustments to Stan settings to meet diagnostic criteria
                        adapt_delta = ifelse(!names(ys[i]) %in% 
                                               c("depth_m_var","depth_m_mean"), 0.98, 0.85),
                        max_treedepth = ifelse(names(ys[i]) == "depth_m_mean", 12, 10),
                        iter_sampling = c(550,500,600,800,600,500,700)[i],
                        refresh = 120)
...
stanfit_i$diagnostic_summary()
# EBFMI 0.39, 0.39, 0.40, 0.42; 0 divergences; 0 maximum tree-depth reached.
...
min(summ$ess_tail,na.rm=TRUE) # 565
max(summ$rhat,na.rm=TRUE)  # 1.018
...
pa_cf_mat[[t]] <- p_cf_mat[[t]] <- predy_cf[[t]]
...
p_cf_mat[[t]] <- 1 - (1 - p_cf_mat[[t]])^4
  ...
  # Predicted presence-absence (1,0) if p > 0.5
pa_cf_mat[[t]][p_cf_mat[[t]] < 0.5] <- 0
pa_cf_mat[[t]][p_cf_mat[[t]] >= 0.5] <- 1
}
# }) # ~15 s
names(p_cf_mat) <- names(pa_cf_mat) <- colnames(biota_ct)
tot_rich_cf_mat <- Reduce('+', pa_cf_mat)
# see https://stackoverflow.com/questions/11641701/sum-a-list-of-matrices
ept_rich_cf_mat <- Reduce('+', pa_cf_mat[substr(names(pa_cf_mat),1,2) %in% c("QE","QP","QT")])
do_rich_cf_mat <- Reduce('+', pa_cf_mat[substr(names(pa_cf_mat),1,2) %in% c("QD","LO")])
...
for(i in 1:nrow(signal)){
  ...
  signal_mat[[i]] <-
    Reduce(function(x) 1-(1-x)^length(ss), pa_cf_mat[ss])
...
y <- apply(p_mean_per_sample,2, FUN = function(x) sum(x >= 0.5))
