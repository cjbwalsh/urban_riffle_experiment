...
The models performed well <!--# Not true for vel_var, cpom and fpom, which all had low BFMI...need to revisit this --> for tests for convergence, effective sample size, and Bayesian Fraction of Missing Information (@stan_dev_team_2022: see Appendix S3 for more details).
...
predy_cf_i <- predy_cf[[ba0cii_neg_taxa[i]]]
...
axis(2, las = 1, at = log(c(0.0001,0.001,0.01,0.1,1,10,100,1000)), 
     labels = ylabs) 
box(bty = "l")
if(taxa$italic[taxa$taxoncode == ba0cii_neg_taxa[i]] == 1){
title(main = bquote(paste(.(LETTERS[i]), # taxa$taxoncode[taxa$taxoncode == ba0cii_taxa[i]],
             ". ", italic(.(taxa$taxon[taxa$taxoncode == ba0cii_neg_taxa[i]])))), 
      adj = 0, cex.main = 1)
}else{
 title(main = bquote(paste(.(LETTERS[i]), # taxa$taxoncode[taxa$taxoncode == ba0cii_taxa[i]],
             ". ", .(taxa$taxon[taxa$taxoncode == ba0cii_neg_taxa[i]]))), 
      adj = 0, cex.main = 1)
