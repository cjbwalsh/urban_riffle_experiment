...
# from urban_riffle_exp_S3.qmd, chunk calc_abundance_fits
load("~/uomShare/wergStaff/ChrisW/git-data/urban_riffle_experiment/model_fits/riffle_baci_oe_stats.rda")
# from urban_riffle_exp_S3.qmd, chunk fig-nbinom_pa_stats
...
<!--# Need to be clear here about which ones were measured in all 6 trips, and which were measured for just 1-4 -->
...
In the laboratory, after macroinvertebrates were collected from the sample, we separated the sample material into fine and coarse particulate organic matter (FPOM, and CPOM, respectively), wood, macrophyte leaves and algae. Each of these elements was dried at 60°C for 24 h and weighed. <!--# Need to dig up lab methods to make sure this is correct -->

We compiled these data into 9 environmental variables to assess the effect of riffles on habitat heterogeneity at the scale of sample units: mean velocity, variance of velocity, mean depth, variance of depth, median $\Phi$ (which showed similar results to minimum and maximum), dry weights of FPOM, CPOM (including wood: it made no difference if it was excluded), macrophytes, and algae. See appendix X for more detail.
...
All $\beta$ parameters were formulated as random effects drawn from community-level hyper-distributions with the mean parameters specified as diffuse normal distributions (mean 0, sd 5) and the parameters for each species drawn from a multivariate normal distribution with a variance-covariance matrix that describes the residual associations among species. Prior distributions for the random effect parameters for site ($\theta_{si}$) and sample ($\theta_{sa}$) normal, with mean 0 and sds drawn from hyper-distributions among sites and samples, respectively, as was the sampling occasion parameter ($\theta_{t}$, among sampling occasions) in the multi-taxon model. For the environmental variable models, prior distribution for $\theta_{t}$ was a normal distribution with mean 0 and sd 1. Broadly, the sd of the random effect distributions were the constraining parameters for model convergence in many models, and we varied their prior distributions among models to maximise sampling convergence (see Appendix S3 for details). For the multi-taxon model, the parameter modelling error among sample units ($\epsilon$), and the dispersion parameter ($\phi$) of the negative binomial distribution for each taxon both had weakly informative normal prior distributions (mean 1, sd 1). We derived the model using the Markov chain Monte Carlo sampler of Stan [@carpenter_etal_2017].

The models satisfied tests for convergence, and effective sample size. The Bayesian Fraction of Missing Information (mean 0.299) was marginally lower than the nominal threshold of 0.3 ( @stan_dev_team_2022: see Appendix S3 for more details).

<!--# Not true for vel_var, cpom and fpom, which all had low BFMI...need to revisit this -->
...
To interpret interactions between the effects of riffle construction and degree of catchment urbanization, we derived posterior predicted distributions of abundances of taxa for which the interactions of interest were non-zero with at least 90% confidence for 12 scenarios. The scenarios comprised four site types (control and impact sites with high and low effective imperviousness (x% and y% EI, the maximum and mimimum EI values in the study sites, respectively), in the three states of the BA effect (ba0, before; ba1, in the first year after riffle construction; and ba2, 5-6 years after).

We focused first on taxa for which the *baci* or *bacii* effects were non-zero with at least 90% confidence to understand which taxa drove assemblage-level differences. We derived predictions of responses of assemblage-level metrics from the model. Responses of diversity measures such as richness and evenness are of intrinsic community ecological interest, but they and biotic metrics focussing on taxa of differing sensitivity to human disturbance are used by stream managers as measures of ecological condition and targets for responses to management actions (e.g. EPA SEPP guidelines?). We used predicted abundancs and occurrences for each scenario to calculate posterior distributions of:
...
-   Pielou's evenness, calculated as $H/log(S)$ where H is the Shannon-Weaver diversity index and S is total taxon richness for each sample.
...
Abundance measures by adding predicted abundances of the relevant taxa. Richness measures by first calculating probability of occurrence in each sample as...(from S3).

For either level of EI, we inferred an effect of riffle construction on *X*, the abundance of a taxon or an assemblage-level metric, if the posterior distribution of

$$
(X_{control,ba0} - X_{control, ba1}) - (X_{impact,ba0} - X_{impact, ba1})
$$

for an effect in the first year, or

$$
(X_{control,ba0} - X_{control, ba2}) - (X_{impact,ba0} - X_{impact, ba2})
$$

for an effect 5-6 years later was non-zero with at least 90% confidence.
...
23 of the 113 taxa responded to riffle construction (non-zero baci differences with 90% confidence; 18 with 95% confidence: @fig-coeff_plot_alt). Only two of those species were negatively associated with EI (i.e. sensitive to human disturbance). One of those, Hydropsychidae, filter-feeding caddis flies which attach to rocks increased in abundance after riffle construction, and only in sites with low EI. The other, a chironomid, *Thiennemaniella*, decreased in abundance, also only in sites with low EI.

Most taxa that were positively associated with EI (i.e. tolerant of human disturbance) decreased in abundance after riffle construction. Nematoda, and oligochaetes *Nais*, *Chaetogaster diaphanus*, Megadrili, Tubificinae gps A and B, *Lumbriculus*, and chironomids *Paratanytarsus/Rheotanytarsus*, *Procladius*, *Parakiefferiella*, *Polypedilum*, and *Cryptochironomus*---most of which inhabit soft sediments---decreased in abundance in low EI sites, but not high EI sites. Only Enchytraidae decreased in both low and high EI sites.

Simuliidae, with similar habitat requirements to Hydropsychidae, but more tolerant of human impacts, increased in abundance in high EI sites but not low EI sites, as did Ceratopogonidae, Glossiphonidae and *Pisidium*.

*Cura pinguis*, *Branchiura sowerbyi*, Cyclopoida, *Cricotopus*, and *Chironomus* increased in abundance in high EI sties, and decreased in low EI sites.

While Hydropsychidae was the only EPT taxon that increased in abundance after riffle construction, EPT richness and abundance increased (only in low EI sites). The predominant decline in abundance of oligochaete and chironomid taxa in low EI sites led to a decrease in QDLO richness and abundance in low EI sites, while responses of *B. sowerbyi*, *Cricotopus*, and *Chironomus* led to an increase in QDLO richness and abundance in high EI sites. The responses of these abundant taxa also explain the complementary trends in evenness (decline in high EI sites, increase in low EI sites).
...
```{r}
#| label: fig-cfplots_bacii_neg
#| echo: false
#| warning: false
#| message: false
#| fig-width: 7
#| fig-height: 6
#| fig-cap: "Trends in abundance of the 8 taxa for which the interaction between the before-after, control-impact and effective imperviousness effects were negative with >90% confidence. The four lines in each plot connect mean (± 95% confidence intervals) metric values  before and at two times after construction of the experimental riffles for control sites (Ctl) and impact (i.e. site in which riffles were placed, Imp) with low and high effective imperviousness (EI)."

# (Plot examples: Hydropsychidae, Thienemanniella, One examples from second group (Lumbriculus?), Enchytraeidae, Simulidae, Ceratopogonidae, [Cura pinguis; Branchia sowerbyi - from last group..different lag?])

taxa_baci <- data.frame(class = c("ba2ci_pos",
                                  rep("ba2ci_neg not ba0cii_neg",sum(!ba2ci_neg_taxa %in% ba0cii_neg_taxa)),
                                  rep("ba2ci_neg and ba0cii_neg", sum(ba2ci_neg_taxa %in% ba0cii_neg_taxa)),
                                  rep("ba0cii_neg not ba2ci_neg",
                                      sum(!ba0cii_neg_taxa %in% c(ba2ci_neg_taxa,ba2ci_pos_taxa)))),
                        taxoncode = c(ba2ci_pos_taxa, 
                                      ba2ci_neg_taxa[!ba2ci_neg_taxa %in% ba0cii_neg_taxa],
                                      ba2ci_neg_taxa[ba2ci_neg_taxa %in% ba0cii_neg_taxa],
                                      ba0cii_neg_taxa[!ba0cii_neg_taxa %in% c(ba2ci_neg_taxa,ba2ci_pos_taxa)])
                        )
...
taxa_set <- taxa_baci$taxoncode[9:16]# non_taxa_baci # 1:8
lo <- layout(matrix(c(9,1:4,9,5:8,0,rep(10,4)),3,5,byrow = TRUE),
       widths = c(1,12,10,10,10),heights = c(10,12,1))
for(i in 1:8){  #Just first 12 out of 19 to start with
