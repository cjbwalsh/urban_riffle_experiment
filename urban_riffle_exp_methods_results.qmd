...
To assess the probability of an effect of riffle construction on assemblage metrics and taxon abundances, we calculated the following four differences between the posterior distributions of metrics and abundances given the 12 scenarios:
...
23 of the 113 taxa responded to riffle construction (non-zero *BA-CI* differences with 90% confidence; 18 with 95% confidence: @fig-coeff_plot_alt). Only two of those species were negatively associated with EI (i.e. sensitive to human disturbance). One of those, Hydropsychidae, filter-feeding caddis flies which attach to rocks increased in abundance after riffle construction, and only in sites with low EI. The other, a chironomid, *Thiennemaniella*, decreased in abundance, only in sites with low EI.

Most taxa that were positively associated with EI (i.e. tolerant of human disturbance) decreased in abundance after riffle construction. Nematoda, and oligochaetes *Nais*, *Chaetogaster diaphanus*, Megadrili, Tubificinae groups A and B, *Lumbriculus variegatus*, Enchytraeidae , and chironomids *Paratanytarsus/Rheotanytarsus*, *Procladius*, *Parakiefferiella*, *Polypedilum*, and *Cryptochironomus*---most of which inhabit soft sediments---decreased in abundance in low EI sites, but not high EI sites. 
...
miny <- 1; maxy <- length(taxa_to_plot)
  ...
  nms <- c("b_i", bquote(Delta~"A1 4%EI"), bquote(Delta~"A2 4%EI"), bquote(Delta~"A1 32%EI"), bquote(Delta~"A2 4%EI"))
for(p in 1:5){
  if(p == 1){
...
taxa_set <- taxa_baci[1:8]# non_taxa_baci # 1:8
...
legend = c("Ctl, 32% EI", "Imp, 32% EI", "Ctl 4% EI","Imp 4% EI"),
cex = 0.75)
}
par(mar = c(0,0,0,0))
plot.new()
